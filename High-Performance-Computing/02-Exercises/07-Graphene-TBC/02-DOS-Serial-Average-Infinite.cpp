...
int main(int argc, char* argv[]) {
    ...
    // Random number generation (dist is by default in [0, 1) which is what we want)
    ...
    // Compute radius to approximate the FBZ of graphene with
    double kmax = 4 * M_PI / (3 * sqrt(3));
    double kmin = 2 * M_PI / 3;
    double kmult = 100;
    ...
    for (unsigned r = 0; r < R; r++) {
        ...
        for (unsigned ell = 0; ell < K; ell++) {
            // Generate random points inside a circle that approximates the FBZ
            // The square root comes from uniform sampling in the unit disk
            double kr = kmult * sqrt(dist(rnd));
            double ktheta = 2 * M_PI * dist(rnd);

            double kx = kr * cos(ktheta);
            double ky = kr * sin(ktheta);


            // Twisting terms (we do an inner product between k and the primitive lattice vector in the direction of the twist)

            // We do k \cdot (sqrt(3) / 2, 3/2)
            for (unsigned n0 = 0; n0 < L; n0++) {
                unsigned here = get_index(n0, L-1, 1, L);

                unsigned up = get_index(n0, 0, 0, L);
                realH(here, up) = std::complex<double> (-t, 0) * std::exp(-I * (sqrt(3)/2 * kx + 3/2 * ky));
                realH(up, here) = std::complex<double> (-t, 0) * std::exp(I * (sqrt(3)/2 * kx + 3/2 * ky));
            }

            // We do k \cdot (sqrt(3), 0)
            for (unsigned n1 = 0; n1 < L; n1++) {
                unsigned here = get_index(L-1, n1, 1, L);

                unsigned right = get_index(0, n1, 0, L);
                realH(here, right) = std::complex<double> (-t, 0) * std::exp(-I * sqrt(3) * kx);
                realH(right, here) = std::complex<double> (-t, 0) * std::exp(I * sqrt(3) * kx);
            }
