...
function buildOpsSubs(spin::Int, states::Vector{Int}, lookup::Dict{Int, Int}, nMMHK::Int, modes::Int, mu::Real, U::Real)
    ...
    # Create the operators (described below inside the loop)
    ops = Dict{String, Matrix{ComplexF64}}()
    ...
    for alpha in 1:nMMHK
        ops["nk_tk_up_$alpha"] = zeros(ComplexF64, dimension, dimension)
        ops["nk_tk_dw_$alpha"] = zeros(ComplexF64, dimension, dimension)
        ops["fmk_z_$alpha"] = zeros(ComplexF64, dimension, dimension)
    end
    ...
    ops["fmk_z"] = zeros(ComplexF64, dimension, dimension)
    ops["afmk_z"] = zeros(ComplexF64, dimension, dimension)
    ...
    for st in states
        ...
        for alpha in 1:nMMHK
            ops["nk_tk_up_$alpha"][ket, ket] = st_list[-1 + 2*alpha] + st_list[-1 + 2*alpha + 2*nMMHK]
            ops["nk_tk_dw_$alpha"][ket, ket] = st_list[2*alpha] + st_list[2*alpha + 2*nMMHK]
        end
    ...
    # Magnetization
    ops["fmk_z"] .= ops["nk_tk_up"] .- ops["nk_tk_dw"]

    # Magnetization per orbital
    for alpha in 1:nMMHK
        ops["fmk_z_$alpha"] .= ops["nk_tk_up_$alpha"] .- ops["nk_tk_dw_$alpha"]
    end
...
"""
function thermal_average(fspace::Array{Subspace}, opcode::String, bbexp::Vector{Float64}, make_positive::Bool = false)
...
function solve(nMMHK::Int, L::Int, mu::Real, U::Real, g::Real, T::Real, delta_start::ComplexF64 = 1e-2 + 0 * im, delta_eps::Real = 1e-4, calpha::Real = 0.8)
    ...
    # Compute actual number of sites in real space
    Nc = Nk * nMMHK
    ...
    afm_z_atomic = Atomic{Float64}(0)

    # Split the k-points in chunks, one for each thread
    numTh = Threads.nthreads()
    chunk_size = cld(Nk, numTh)
    chunks = collect(Iterators.partition(kk, chunk_size))

    # Create one deepcopy of fspace per chunk
    local_fspaces = [deepcopy(fspace) for _ in 1:length(chunks)]
    ...
    while keep_going
        ...
        @threads for i in eachindex(chunks)
            ...
            # Copying the needed data
            ch = chunks[i]
            fspace_local = local_fspaces[i]
            ...
            for k in ch
                ...
                for sub in fspace_local
                    ...
                    # Overwrite the tight-binding matrix for efficiency (mutation)
                    sub.ops["hamTB"] .+= sub.ops["hamMU"] .+ sub.ops["hamHK"] .+ (delta_start' .* sub.ops["bk_tk"] .+ delta_start .* sub.ops["bk_tk"]')

                    # TODO: Delete this XD
                    # Add an AFM field to check our operators work
                    sub.ops["hamTB"] += -10 * sub.ops["afmk_z"]
                ...
                index_vals = 1
                for sub in fspace_local
                    newIndex = index_vals + sub.dimension - 1
                    valsOverwrite[index_vals:newIndex] = sub.vals
                    index_vals = newIndex + 1
                end

                # Scaled energies
                mini = minimum(valsOverwrite)

                # Boltzmann exponential
                # If T = 0 then if E = 0 the exponent is 1 and otherwise is zero
                if isapprox(T, 0.0, atol=1e-14)
                    for i in eachindex(valsOverwrite)
                        valsOverwrite[i] = isapprox((valsOverwrite[i] - mini), 0.0, atol=1e-14) ? 1.0 : 0.0
                    end
                else
                    for i in eachindex(valsOverwrite)
                        valsOverwrite[i] = exp(-(valsOverwrite[i] - mini) / T)
                    end
                end
                bbexp = valsOverwrite
                ...
                # Compute Delta
                deltaLocal += thermal_average(fspace_local, "bk_tk", bbexp)

                # This is the last lap, compute outputs
                if !keep_going
                    nLocal += real(thermal_average(fspace_local, "nk_tk_ts", bbexp))
                    afm_zLocal += real(thermal_average(fspace_local, "afmk_z", bbexp, true))
                    ...
                    KxxLocal += real(thermal_average(fspace_local, "hamTB", bbexp))
                end
            end
            ...
            if !keep_going
                ...
                atomic_add!(afm_z_atomic, afm_zLocal)
            end
        end
...
## ----- MAIN CODE -----
runID = floor(Int, parse(Float64, ARGS[1]))
nMMHK = floor(Int, parse(Float64, ARGS[2]))
L = floor(Int, parse(Float64, ARGS[3]))
