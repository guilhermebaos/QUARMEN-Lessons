...
function solve(nMMHK::Int, L::Int, mu::Real, U::Real, g::Real, T::Real, delta_start::ComplexF64 = 1e-2 + 0 * im, delta_eps::Real = 1e-4, calpha::Real = 0.8)
    ...
    afm_z_sq_atomic = Atomic{Float64}(0)
        ...
        @threads for i in eachindex(chunks)
            ...
            for k in ch
                ...
                if !keep_going
                    ...
                    afm_z_sqLocal += real(thermal_average(fspace_local, "afmk_z", bbexp, true))
            ...
            if !keep_going
                ...
                atomic_add!(afm_z_sq_atomic, afm_z_sqLocal)
            end
        end
    ...
    n = n_atomic[]
    Kxx = Kxx_atomic[]
    afm_z_sq = afm_z_sq_atomic[]

    # Normalize
    n *= 1 / (2 * Nk * nMMHK)
    Kxx *= -pi / (2 * Nk * nMMHK)
    afm_z_sq *= 1 / (2 * Nk * nMMHK)
