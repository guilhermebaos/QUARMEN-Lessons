...
# PARAMETERS
L = 1000
g = int(script_name.split("-")[1]) / 100 * W
T = 1e-9 * W

# Hubbard 
uu = np.array([0.8, 1.2]) * W
...
for U in uu:
    ...
    np.savez(script_out / f"{d}={L}={T}={g}={U}", xx_mesh=xx_mesh, jj_mesh=jj_mesh, dd_mesh=dd_mesh, mm_mesh=mm_mesh, pp_mesh=pp_mesh)
